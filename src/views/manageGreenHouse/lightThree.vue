...
const showReport = async () => {
    ...
    try {
        if (model.value == true) {
            const lightOne = await axios.post(apiUrl + 'auth/publish_on_off_light_three',
                {
            ...
            $toast.warning("Light 3 turn off");
...
<template>
        ...
        <v-card-text style="flex-grow: 1; padding: 0;">
            <div class="d-flex align-start mb-6" style="height: 100%;">
                    <v-card width="100%">
                        <v-switch v-model="model" :label="`Light 3 status: ${model.toString()}`" hide-details inset
