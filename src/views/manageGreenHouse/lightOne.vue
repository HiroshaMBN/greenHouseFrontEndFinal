...
const showReport = async () => {
    ...
    try {
        if (model.value == true) {
            const lightOne = await axios.post(apiUrl + 'auth/publish_on_off_light_one',
                {
            ...
            $toast.warning("Light 1 turn off");
...
<template>
    ...
    <v-card-text style="flex-grow: 1; padding: 0;">
        <div class="d-flex align-start mb-6" style="height: 100%;">
            <v-card width="100%">
                <!-- Use @update:model-value to update 'model' reactively -->
                <v-switch v-model="model" :label="`Light 1 status: ${model.toString()}`" hide-details inset
