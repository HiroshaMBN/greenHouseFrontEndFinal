...
const initializeChart = () => {
    const options = {
        ...
        series: [{
            name: 'Soil',
            data: series.value.monthDataSeries1.soil,
        }], colors: ['#d1480d'],
    };
...
const updateChart = () => {
    const chart = ApexCharts.getChartByID('SoilChart');
    if (chart) {
        chart.updateOptions({
            labels: series.value.monthDataSeries1.dates,
            series: [{
...
const showReport = async () => {
    ...
    try {
        ...
        if (res.data.status === 404) {
            ...
            const data = res.data as Array<{ created_at: string; soil: number }>;
            ...
            series.value.monthDataSeries1.soil = soil;

            updateChart();

            $toast.success(`Soil Report updated between ${startDate.value} and ${endDate.value}`);
        }
...
<template>
    <v-card elevation="0" class="overflow-hidden bubble-secondary-shape"
        ...
        <v-text style="text-align: center;">Soil Report</v-text>
        ...
        <v-card-text style="flex-grow: 1; padding: 0;">
            ...
            <div class="d-flex align-start mb-6" style="height: 100%;">
                ...
                <div class="mt-8" style="height: 100%; width: 100%;"> <!-- Ensuring this container takes full space -->
                    ...
                    <div id="SoilChart" style="height: 100%; width: 100%;"></div> <!-- Full height and width -->
                </div>
            </div>
        </v-card-text>
    </v-card>
