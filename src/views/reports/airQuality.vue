...
const initializeChart = () => {
    const options = {
        ...
        series: [{
            name: 'Air quality',
            data: series.value.monthDataSeries1.air,
        }], colors: ['#18f005'],
    };

    // Create and render the chart
    const chart = new ApexCharts(document.querySelector('#AirQualityChart'), options);
    chart.render();
};

// Chart update function
const updateChart = () => {
    const chart = ApexCharts.getChartByID('AirQualityChart');
    if (chart) {
        chart.updateOptions({
            labels: series.value.monthDataSeries1.dates,
            series: [{
...
const showReport = async () => {
    ...
    try {
        ...
        if (res.data.status === 404) {
            ...
            const data = res.data as Array<{ day: string; air: number }>;
            // Extract and update series
            const dates = data.map(item => item.day);
            const air = data.map(item => item.air);
            

            series.value.monthDataSeries1.dates = dates;
            series.value.monthDataSeries1.air = air;
    
            updateChart();

            $toast.success(`Air quality report updated between ${startDate.value} and ${endDate.value}`);
        }
...
<template>
    <v-card elevation="0" class="overflow-hidden bubble-secondary-shape"
        ...
        <v-text style="text-align: center;">Air Quality Report</v-text>
        ...
        <v-card-text style="flex-grow: 1; padding: 0;">
                ...
                <v-row>
                    ...
                    <v-col cols="3">
                        <v-select label="Select type" :items="typeOptions" item-title="text" item-value="value"
                            variant="underlined" v-model="type" required></v-select>
                    </v-col>
                    <v-col cols="3">
            ...
            <div class="d-flex align-start mb-6" style="height: 100%;">
                ...
                <div class="mt-8" style="height: 100%; width: 100%;"> <!-- Ensuring this container takes full space -->
                    ...
                    <div id="AirQualityChart" style="height: 100%; width: 100%;"></div> <!-- Full height and width -->
                </div>
            </div>
        </v-card-text>
    </v-card>
